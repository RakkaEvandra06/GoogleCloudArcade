...
import {
  getFacilitatorMembers,
  addFacilitatorMember,
  removeFacilitatorMember,
  getParticipantByUrl,
  addParticipant,
  createAuditLog,
} from '@/lib/db';
...
async function authFac(_req: Request) {
  const s = await getSession();
  return s?.role === 'facilitator' ? s : null;
}
...
export async function POST(req: Request) {
  ...
  // Rate limit
  if (!checkRateLimit(`fac-add:${ip}`, 20, 60))
  ...
  if (!validateOrigin(req)) {
    logSecurity('warn', 'ssrf_attempt', { reason: 'csrf-fac-members-post' });
...
export async function DELETE(req: Request) {
  // CSRF / origin guard (no body for DELETE, so no CT guard needed)
  if (!validateOrigin(req)) {
    logSecurity('warn', 'ssrf_attempt', { reason: 'csrf-fac-members-delete' });
    return NextResponse.json({ error: 'Forbidden.' }, { status: 403 });
  }
