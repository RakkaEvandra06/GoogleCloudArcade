...
/** Basic RFC-5322 surface check — rejects placeholders and obviously malformed addresses */
function isValidEmail(email: string): boolean {
  if (!email) return false;
  if (email.includes('placeholder')) return false;       // catch dev fallbacks
...
export async function POST(req: Request) {
  ...
  // override_email must be a valid address when provided — the caller sends one
  // address to BCC-blast all selected participants (e.g. a facilitator CC list).
  // For per-participant addresses, participants need an email field in the DB.
  ...
  for (const id of ids) {
    ...
    try {
      ...
      // Prefer override_email (facilitator-supplied); fall back to the
      // participant's stored email once that field exists in the DB schema.
      // Previously this fell back to `uuid-prefix@placeholder.com` — a live
      // Resend API call to a fake domain that bounces and wastes credits.
      const recipientEmail = overrideEmail || (p as unknown as Record<string, unknown>).email as string | undefined;

      if (!recipientEmail || !isValidEmail(recipientEmail)) {
        // Skip rather than send to a placeholder address
        skipped++;
        continue;
      }
