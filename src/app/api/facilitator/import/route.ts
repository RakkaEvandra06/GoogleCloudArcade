...
export async function POST(req: Request) {
  ...
  // Rate limit
  if (!checkRateLimit(`fac-import:${ip}`, 3, 300))
  ...
  if (!validateOrigin(req)) {
    logSecurity('warn', 'ssrf_attempt', { reason: 'csrf-fac-import' });
  ...
  if (!Array.isArray(body.rows))
    return NextResponse.json({ error: '`rows` must be an array of URL strings.' }, { status: 400 });

  const rows     = (body.rows as unknown[]).slice(0, MAX_IMPORT_ROWS).map(String);
  ...
  for (const rawUrl of rows) {
    ...
    try {
      ...
      if (!p) {
        ...
        const sr = await fetch(`${base}/api/scrape?url=${encodeURIComponent(url)}`, {
          cache: 'no-store',
        ...
        if (sr.ok) {
          ...
          await updateParticipant(p.id, { name: d.name || '', avatar_url: d.avatar_url, last_synced: d.scraped_at });
        }
      }
      ...
      failed++;
      results.push({ url, status: 'error', message: 'Processing failed.' });
    }
